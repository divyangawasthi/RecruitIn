...
def submit(root):
    ...
    if email1 and password:
        hashed_pass = hash_password(str(password))
        for i in total:
            if email1 == i[0] and hashed_pass == i[1]:
                return success(root, email1)
            elif email1 == i[0] and hashed_pass != i[1]:
