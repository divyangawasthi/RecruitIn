...
from modules.password_hasher import *
...
def mai(root):
    ...
    recruit = Button(r1, image=r1.Img1, border=0, bg="#6895D8",
                     relief="raised", activebackground="#FFDE59", command=lambda: recruiter_regis(root))
    recruit.place(x=540, y=280)
    r1.Img2 = PhotoImage(file="elements/client_element.png")
    recruit2 = Button(r1, image=r1.Img2, border=0, bg="#6895D8",
                      relief="raised", activebackground="#FFDE59", command=lambda: client_regis(root))
    recruit2.place(x=760, y=280)
    r1.bn = PhotoImage(file="elements\\backlogin.png")
    btn = Button(r1, image=r1.bn, bg='#6895D8',
                 bd=0, activebackground="#FFFFFF", command=lambda: logi(root))
    btn.place(x=630, y=475)
...
def recruiter_regis(root):
    ...
    img.place(x=0, y=0)
    name_l = Label(r2, text="Name : ", bg='#329DF3', fg="#FFDE59",
                   font=('normal', 20, 'bold'))
    name_l.place(x=600, y=250)
    name = Entry(r2, placeholder='Enter Your Full Name...', width=20)
    name.place(x=790, y=250)

    email_l = Label(r2, text="Email : ", bg='#329DF3', fg="#FFDE59",
                    font=('normal', 20, 'bold'))
    email_l.place(x=600, y=300)
    email = Entry(r2, placeholder='Email', width=20)
    email.place(x=790, y=300)

    pwd_l = Label(r2, text="Password : ", bg='#329DF3', fg="#FFDE59",
                  font=('normal', 20, 'bold'))
    pwd_l.place(x=600, y=350)
    pwd = Entry(r2, placeholder='Password', show="*", width=20)
    pwd.place(x=790, y=350)

    con_pwd_l = Label(r2, text="Confirm : ", bg='#329DF3', fg="#FFDE59",
                      font=('normal', 20, 'bold'))
    con_pwd_l.place(x=600, y=400)
    cpwd = Entry(r2, placeholder='Confirm Password', show="*", width=20)
    cpwd.place(x=790, y=400)

    r2.bn = PhotoImage(file="elements\\next1.png")
    btn = Button(r2, image=r2.bn, bg='#329DF3', bd=0,
    ...
    btn2 = Button(r2, image=r2.back, bg='#329DF3', bd=0,
                  activebackground="#FFFFFF", command=lambda: mai(root))
    btn2.place(x=620, y=500)


def recruiter_check(root):
    global name1, email1, pwd1, cpwd1, hashed_pwd1, hashed_cpwd1
    name1 = name.get()
    email1 = email.get()
    pwd1 = pwd.get()
    cpwd1 = cpwd.get()
    hashed_pwd1 = hash_password(str(pwd1))
    hashed_cpwd1 = hash_password(str(cpwd1))
...
def recruiter_submit(root):
    ...
    if gender1 and company1 and loc1:
        print(pwd1)
        print(hashed_pwd1)
        exe = f'insert into users values("{name1}","{email1}","recruiter","{hashed_pwd1}")'
...
def client_submit(root):
    ...
    if gender1 and age1 and loc1 and workxp1:
        exe = f'insert into users values("{name1}","{email1}","client","{hashed_pwd1}")'
