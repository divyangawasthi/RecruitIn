...
#-----------------------------------------------EDIT A JOB------------------------------------------------
def editjob(table):
    global role2, jtype2, qual2, exp2, sal2, ajid
    selectedindex = table.focus()
    selectedvalues = table.item(selectedindex, 'values')
    ajid = selectedvalues[0]

    q = f'SELECT JobRole, JobType, Qualification, MinExp, Salary from mydb.job where JID="{ajid}";'
    mycon = sql.connect(host='localhost', user='root',
                        passwd=user_pwd, database='mydb')
    cur = mycon.cursor()
    cur.execute(q)
    ...
    # Create Form
    f1 = Frame(rt, width=520)
    f1.load = PhotoImage(file="elements\\edit_job.png")
    img = Label(rt, image=f1.load, bg="#FFFFFF")
    img.grid(row=0, column=1, padx=150, pady=10)
    ...
    role2 = Entry(tab, placeholder="Enter Job Role")
    role2.insert(jobRole)
    role2.grid(row=0, column=1, pady=10, padx=10)
    jtype2 = ttk.Combobox(tab, font=("normal", 18),
                         width=23, state='readonly')
    jtype2['values'] = ('Select', 'FullTime', 'PartTime', 'Intern')

    lst = list(jtype2['values'])
    for i in range(len(lst)):
        if lst[i] == jobType:
            index = i
            break

    jtype2.current(index)


    jtype2.grid(row=1, column=1, pady=10, padx=10)
    qual2 = Entry(tab, placeholder="Enter Job Qualifications")
    qual2.insert(qualification)
    qual2.grid(row=2, column=1, pady=10, padx=10)
    exp2 = Entry(tab, placeholder="Enter Minimum Experience")
    exp2.insert(minExp)
    exp2.grid(row=3, column=1, pady=10, padx=10)
    sal2 = Entry(tab, placeholder="Enter Expected salary")
    sal2.insert(salary)
    sal2.grid(row=4, column=1, pady=10, padx=10)

    btn = Button(tab, text="Submit", font=(20), bg="#45CE30",
                 fg="#FFFFFF", command=lambda:update_job())
    btn.grid(row=5, column=1, pady=15)
...
def update_job():
    ...
    if role1 and jtype1 and qual1 and exp1 and sal1:
        if jtype1 == "Select":
            ...
            exe1 = f'UPDATE mydb.Job SET JobRole = "{role1}", JobType = "{jtype1}", Qualification = "{qual1}", MinExp = {exp1}, Salary = {sal1} WHERE JID = {ajid};'
            try:
                mycon = sql.connect(host='localhost', user='root',
                                    passwd=user_pwd, database='mydb')
                cur = mycon.cursor()
                cur.execute(exe1)
                messagebox.showinfo('SUCCESS!', 'You have successfully updated the Job')
                posted()
                role.delete(0, END)
                jtype.delete(0, END)
                qual.delete(0, END)
...
def posted():
    ...
    global search_d
    search_d = ttk.Combobox(rt, width=8, font=(
        'normal', 10), state='readonly')
    ...
    edt = Button(rt, text="Edit", font=('normal', 12, 'bold'),
                 bg="#00b9ed", fg="#ffffff", command=lambda: editjob(table))
    edt.grid(row=0, column=4, padx=10, pady=10, ipadx=15)
...
def app():
    ...
    table.column("JobRole", width=100, anchor=CENTER)
    table.column("CName", width=200, anchor=CENTER)
    table.column("CEmail", width=200, anchor=CENTER)
    table.column("CAge", width=50, anchor=CENTER)
    table.column("CLocation", width=150, anchor=CENTER)
    table.column("CGender", width=100, anchor=CENTER)
    table.column("CExp", width=100, anchor=CENTER)
    table.column("CSkills", width=200, anchor=CENTER)
    table.column("CQualification", width=150, anchor=CENTER)
...
def rec(root, email1):
    ...
    # Left
    lf = Frame(root, width=230, height=440, bg="#ffffff")
    lf.place(x=60, y=220)

    # lf.bn = PhotoImage(file="elements\\reg.png")
    # cj = Button(lf, image=lf.bn, bg='#329DF3', bd=0,
    #              activebackground="#ffffff", command=create)
